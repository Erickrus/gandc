#!/usr/bin/env python3
"""
ACP REPL — A Python REPL that communicates with opencode via the Agent Client Protocol.

Usage:
    python3 acp_repl.py [--cwd /path/to/project] [--opencode-bin opencode]

Spawns `opencode acp` as a subprocess and communicates over JSON-RPC/ndJSON on stdio.
"""
...
class ACPClient:
    def __init__(self, opencode_bin: str, cwd: str, debug: bool = False):
    ...
    def start(self):
        ...
        self.proc = subprocess.Popen(
            [self.opencode_bin, "acp", "--cwd", self.cwd],
    ...
    def _handle_message(self, msg: dict):
        ...
        # Handle session/update (can come as notification or request)
        ...
        # Respond to any unknown request so the agent doesn't hang
...
def main():
    parser = argparse.ArgumentParser(description="ACP REPL for opencode")
    ...
    client = ACPClient(args.opencode_bin, args.cwd, debug=args.debug)
...
\\____/ .___/\\___/_/ /_/\\___/\\____/\\__,_/\\___/   \\__,_/\\___/ .___/ 
    /_/                                                  /_/
{RESET}{DIM}            ACP Client — CLI {RESET}
""")
    print(f"Connecting to opencode...")
    client.start()
    ...
    while True:
        ...
        if stripped.lower() == "/mcps":
            ...
            continue
        # Invoke skill if it matches an available command
    ...
    print(f"{DIM}Resume with: python3 acp_repl.py --session {client._session_id}{RESET}")
